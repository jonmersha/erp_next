...
const Users: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const uData = await fetchCollection<UserProfile>('erp-users', profile.companyId);
        setUsers(uData);
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await apiService.updateDocument('erp-users', userId, { 
  ...
  const handleDeleteUser = async (user: UserProfile) => {
    ...
    try {
      await apiService.deleteDocument('erp-users', user.uid);
      const uData = await fetchCollection<UserProfile>('erp-users', profile.companyId);
      setUsers(uData);
    } catch (err: any) {
