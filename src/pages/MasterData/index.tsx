...
const MasterData: React.FC = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL !== undefined ? process.env.NEXT_PUBLIC_API_BASE_URL.replace(/\/$/, '') : 'https://milkitest.besheger.com'}/api/${endpoint}/template`, {
  ...
  const handleUploadCsv = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const endpoint = activeTab === 'raw' ? 'rawMaterials' : 'products';
      const token = await auth.currentUser?.getIdToken() || '';
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL !== undefined ? process.env.NEXT_PUBLIC_API_BASE_URL.replace(/\/$/, '') : 'https://milkitest.besheger.com'}/api/${endpoint}/upload`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });
              ...
              {activeTab === 'products' && products.map(p => (
                <tr key={p.id} className="hover:bg-[var(--color-text)]/[0.02] transition-colors">
                  <td className="px-6 py-4 font-bold text-[var(--color-text)] flex items-center gap-3">
                    {p.imageUrl ? (
                      <img crossOrigin="anonymous" src={apiService.getImageUrl(p.imageUrl)} alt={p.name} className="w-10 h-10 rounded-lg object-cover border border-[var(--color-text)]/10" />
                    ) : (
