...
const HR: React.FC = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL !== undefined ? process.env.NEXT_PUBLIC_API_BASE_URL.replace(/\/$/, '') : 'https://milkitest.besheger.com'}/api/employees/template`, {
  ...
  const handleUploadCsv = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const token = await auth.currentUser?.getIdToken() || '';
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL !== undefined ? process.env.NEXT_PUBLIC_API_BASE_URL.replace(/\/$/, '') : 'https://milkitest.besheger.com'}/api/employees/upload`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });
