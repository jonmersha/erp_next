...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: ShoppingCart,
      submenu: [
        { name: t('Procurement Operations'), path: '/procurement', icon: ShoppingCart, module: 'procurement' },
  ...
  useEffect(() => {
    ...
    if (activeParent) {
      // By default the menus are collapsed, so we don't auto-expand them.
  ...
  return (
    <div className="flex h-screen bg-[var(--color-bg)] text-[var(--color-text)] overflow-hidden">
      ...
      <aside className={`w-64 bg-[var(--color-shell)] text-[var(--color-shell-text)] hidden flex-col h-full shadow-lg z-20 flex-shrink-0 nav-dropdown-container ${isDesktopSidebarOpen ? 'md:flex' : ''}`}>
        <div className="w-full px-6 h-16 flex items-center border-b border-black/10 flex-shrink-0">
          <div className="flex items-center space-x-3">
            {company?.logoUrl ? (
              <img crossOrigin="anonymous" src={apiService.getImageUrl(company.logoUrl)} alt="Company Logo" className="h-8 w-8 object-contain rounded bg-white/10" referrerPolicy="no-referrer" />
            ) : (
            ...
            <span className="text-xl font-bold tracking-tight text-white truncate">
              {company?.name || 'Sheger ERP - Milki Food Complex Test'}
            </span>
          </div>
        </div>
            ...
            <div key={item.name} className="w-full">
              {item.submenu ? (
                <button
                  ...
                  className={`flex items-center justify-between w-full px-6 py-3 text-sm transition-colors ${openSubmenu === item.name ? 'bg-black text-white border-l-4 border-white' : 'text-white/80 hover:bg-[var(--color-shell-hover)] hover:text-white border-l-4 border-transparent'}`}
                >
                  <div className="flex items-center space-x-3">
                    <item.icon size={18} />
                    <span>{item.name}</span>
                ...
                <Link
                  href={item.path!}
                  className={`flex items-center space-x-3 px-6 py-3 text-sm transition-colors ${pathname === item.path ? 'bg-black text-white border-l-4 border-white' : 'text-white/80 hover:bg-[var(--color-shell-hover)] hover:text-white border-l-4 border-transparent'}`}
                >
                  <item.icon size={18} />
                  <span>{item.name}</span>
                </Link>
                ...
                <div className="bg-[var(--color-shell-hover)]/50 py-1">
                  {item.submenu.map(sub => (
                    <Link
                      key={sub.path}
                      href={sub.path}
                      className={`flex items-center space-x-3 pl-12 pr-6 py-2.5 text-sm transition-colors ${pathname === sub.path ? 'bg-black text-white font-bold' : 'text-white/70 hover:text-white hover:bg-[var(--color-shell-hover)]'}`}
                    >
                      <sub.icon size={16} />
                      <span>{sub.name}</span>
                    </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="bg-[var(--color-surface)] text-[var(--color-text)] border-b border-[var(--color-border)] sticky top-0 z-10 shadow-sm flex-shrink-0">
          <div className="w-full px-4 h-16 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                {company?.logoUrl ? (
                  <img crossOrigin="anonymous" src={apiService.getImageUrl(company.logoUrl)} alt="Company Logo" className="h-8 w-8 object-contain rounded border border-[var(--color-border)]" referrerPolicy="no-referrer" />
                ) : (
            ...
            {/* Tablet/Mobile Tools */}
            <div className="flex flex-wrap justify-end items-center gap-1 md:gap-2">
              <div className="lang-dropdown-container relative group flex items-center">
                <button
                  onClick={() => setIsLangMenuOpen(!isLangMenuOpen)}
                  className="p-1 md:p-2 text-[var(--color-text)]/80 hover:bg-[var(--color-text)]/5 rounded-full transition-colors flex items-center"
              ...
              <button onClick={() => setIsHelpOpen(true)} className="p-1 md:p-2 text-[var(--color-text)]/80 hover:bg-[var(--color-text)]/5 rounded-full transition-colors" title={t('Help')}>
                <HelpCircle size={18} className="md:w-5 md:h-5" />
              </button>
              <button onClick={toggleTheme} className="p-1 md:p-2 text-[var(--color-text)]/80 hover:bg-[var(--color-text)]/5 rounded-full transition-colors" title={t('Toggle Theme')}>
                {theme === 'light' ? <Moon size={18} className="md:w-5 md:h-5" /> : <Sun size={18} className="md:w-5 md:h-5" />}
              </button>
              <Link href="/profile" className="flex items-center space-x-2 p-1 cursor-pointer hover:bg-[var(--color-text)]/5 rounded-full transition-colors" title={`${profile?.name} - View Profile`}>
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[var(--color-main)] flex items-center justify-center text-white text-xs md:text-sm font-bold shadow-sm hover:scale-105 transition-transform">
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 left-0 h-full w-64 bg-[var(--color-shell)] z-50 flex flex-col md:hidden pt-8 shadow-xl"
              >
                {/* Mobile Drawer Header with Logo & Name */}
                <div className="w-full px-6 pb-6 flex items-center border-b border-white/10 flex-shrink-0">
                  <div className="flex items-center space-x-3">
                    {company?.logoUrl ? (
                      <img crossOrigin="anonymous" src={apiService.getImageUrl(company.logoUrl)} alt="Company Logo" className="h-8 w-8 object-contain rounded bg-white/10" referrerPolicy="no-referrer" />
                    ) : (
                ...
                <nav className="flex flex-col h-full overflow-y-auto nav-dropdown-container pt-2">
                  {filteredNavItems.map((item) => (
                    <div key={item.name} className="w-full">
                      {item.submenu ? (
                        <button
                          onClick={() => setOpenSubmenu(openSubmenu === item.name ? null : item.name)}
                          className={`flex items-center justify-between w-full px-6 py-4 text-sm transition-colors ${openSubmenu === item.name ? 'bg-black text-white' : 'text-white/80 hover:bg-[var(--color-shell-hover)]'}`}
                        >
                          <div className="flex items-center space-x-3">
                            <item.icon size={18} />
                            <span>{item.name}</span>
                        ...
                        <Link
                          href={item.path!}
                          className={`flex items-center space-x-3 px-6 py-4 text-sm transition-colors ${pathname === item.path ? 'bg-black text-white' : 'text-white/80 hover:bg-[var(--color-shell-hover)]'}`}
                        >
                          <item.icon size={18} />
                          <span>{item.name}</span>
                        </Link>
                        ...
                        <div className="bg-[var(--color-shell-hover)]">
                          {item.submenu.map(sub => (
                            <Link
                              key={sub.path}
                              href={sub.path}
                              className={`flex items-center space-x-3 px-10 py-3.5 text-sm text-left transition-colors ${pathname === sub.path ? 'bg-black text-white font-bold' : 'text-white/70 hover:text-white hover:bg-[var(--color-shell-hover)]'}`}
                            >
                              <sub.icon size={16} />
                              <span>{sub.name}</span>
                            </Link>
