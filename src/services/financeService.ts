...
export const approveFinancialPlan = async (id: string, approverId: string) => {
  return await apiService.post(`plans/financial/${id}/approve`, { approverId });
};

export const rejectFinancialPlan = async (id: string, approverId: string, rejectionReason: string) => {
  return await apiService.post(`plans/financial/${id}/reject`, { approverId, rejectionReason });
};
